...
<ul class="pb-2 space-y-2">
    ...
    <li class="mt-2">
        <a href="{{ route('admin.index') }}" class="flex items-center p-2 text-sm text-gray-900 hover:font-semibold rounded-lg hover:bg-gray-100 group dark:text-gray-200 dark:hover:bg-gray-700
            ...
            <svg class="w-6 h-6 text-gray-400 transition duration-75 svg-icon group-hover:text-gray-900 dark:text-gray-400 dark:group-hover:text-white
    ...
    <li>
        <div id="accordion-color" data-accordion="collapse" data-active-classes="dark:bg-gray-800 dark:text-white">
            <h2 id="accordion-color-heading-1">
                <button type="button" class="flex items-center w-full p-2 text-base text-gray-900 transition duration-75 rounded-lg group hover:bg-gray-100 dark:text-gray-200 dark:hover:bg-gray-700" data-accordion-target="#accordion-color-body-1" aria-expanded="{{ Request::is('dashboard/admin/regions*')? 'true' : 'false' }}" aria-controls="accordion-color-body-1">
                    <svg class="w-6 h-6 text-gray-400 transition duration-75 svg-icon group-hover:text-gray-900 dark:text-gray-400 dark:group-hover:text-white" xmlns=" http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
            ...
            <div id="accordion-color-body-1" class="hidden" aria-labelledby="accordion-color-heading-1">
                <div>
                    <ul class="py-2 space-y-1 ">
                        ...
                        <li>
                            <a href="{{ route('admin.region', $region->slug) }}" class="flex items-center p-2 text-base text-gray-900 transition duration-75 rounded-lg pl-11 group hover:bg-gray-100 dark:text-gray-200 dark:hover:bg-gray-700
                                            {{ Request::is('dashboard/admin/regions/'.$region->slug)? 'bg-gray-100  dark:bg-gray-700 dark:text-gray-200' : '' }}"> {{ $region->name   }}</a>
                        </li>
                        @endforeach
                    </ul>
                </div>
