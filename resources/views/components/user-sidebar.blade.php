<ul class="space-y-2 font-medium">
    <!-- DASHBOARD -->
    <li>
        <a href="{{ route('user.index') }}" class="flex items-center p-2 text-sm text-gray-900 hover:font-semibold rounded-lg hover:bg-gray-100 group d
                        {{ Request::is('dashboard/user')? 'bg-gray-100 font-semibold ' : '' }}">
            <svg class="w-6 h-6 text-gray-500 transition duration-75 svg-icon group-hover:text-gray-900
                            {{ Request::is('dashboard/user')? 'text-gray-900 ' : '' }}" fill="currentColor" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg">
    ...
    <li>
        <a href="{{ route('user.show', $user->username) }}" class="flex items-center p-2 text-sm text-gray-900 hover:font-semibold rounded-lg hover:bg-gray-100 group
                        {{ Request::is('dashboard/user/*') ? 'bg-gray-100 font-semibold ' : '' }}
                        ">
            <svg class="w-6 h-6 text-gray-500 transition duration-75 svg-icon group-hover:text-gray-900
                            {{ Request::is('dashboard/user/*')? 'text-gray-900 ' : '' }}" fill="currentColor" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg">
    ...
    <li>
        <form action="{{ route('user.logout') }}" method="post">
            @csrf
            <button class="w-full px-4 py-2 my-1 text-center text-white bg-red-500 rounded-lg shadow-md">
                Sign Out
            </button>
        </form>
    </li>
